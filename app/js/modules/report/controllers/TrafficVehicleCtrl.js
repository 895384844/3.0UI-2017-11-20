define(function() {
	return ['$scope', 'i18nService', '$lt', '$filter', 'GridServiceReport', 'ModalService', '$element', 'localSession', 'HttpService', 'DialogService', 'SystemService', 'DateFormat', 'EmptyInput', 'delEmptyInput','groupSelect', 'uiGridConstants', TrafficVehicleCtrl];

	function TrafficVehicleCtrl($scope, i18nService, $lt, $filter, GridServiceReport, ModalService, $element, localSession, HttpService, DialogService, SystemService, DateFormat, EmptyInput, delEmptyInput,groupSelect, uiGridConstants) {
		...
		$scope.exports = function(){
			...
			var filter = {
				title: "车辆流量分析-"+times,
			    fields: field,
			    fileName: "TrafficVehiclePageList-"+times+".xlsx",
			    sheetName: "Sheet1",
			    param: {
			...
			HttpService.post('rest/analysis/vehicle/traffic_export',filter).then(function success(resp) {
				$scope.showLoading = false;
		...
		function showCharts() {
			//	var filter={page_size:pageSize,page_no:currentPage-1};
	            ...
	            var result = HttpService.post('rest/analysis/vehicle/traffic',filter);
