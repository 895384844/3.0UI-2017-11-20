define(['moment', 'echarts'], function (moment, echarts) {
	return ['$scope', 'HttpService','GridService','ModalService','DateFormat', 'delEmptyInput','EmptyInput', 'DialogService',VehicleCollisionCtrl];

	function VehicleCollisionCtrl($scope, HttpService,GridService,ModalService,DateFormat,delEmptyInput,EmptyInput,DialogService) {
		$scope.btn = {
	        status : true
	    }
		...
		var exports = function(){
			...
			var filter = {
				title: "车牌碰撞分析-"+times,
			    fields: field,
			    fileName: "VehicleCollisionPageList-"+times+".xlsx",
			    sheetName: "Sheet1",
			    param: {
			...
			HttpService.post('rest/analysis/vehicle/collision_export',filter).then(function success(resp) {
				$scope.showLoading = false;
		...
		GridService.create($scope,{
				...
				columnDefs:[					
		            ...
		            { field: 'plateColor',displayName: '车牌颜色',enableSorting:false,enableColumnMenu:false },
		            { field: 'vehicleColor',displayName: '车身颜色',enableSorting:false,enableColumnMenu:false },
		            { field: 'traceCount',displayName: '查询结果总数',enableSorting:false,enableColumnMenu:false },
		            { field: 'deviceCount',displayName: '经过设备点位个数',enableHiding:false,enableSorting:false,enableColumnMenu:false },
		            { field: 'attributeName',displayName: '归属地',enableHiding:false,enableSorting:false,enableColumnMenu:false },
		            { 
		                field: 'btnGroup',
		                displayName: '查看轨迹',
		                enableSorting:false,
		...
		$scope.showMap = function(row) {
            ...
            HttpService.post('rest/analysis/vehicle/collisionOnMap',filter).then(function success(resp){
            	$scope.showLoading = false;
            	var locations = [];
				var pt = [];
		...
		$scope.search = function(){
			...
			$scope.getPagingList = function(currentPage, pageSize, sort) {				
	            ...
	            var result = HttpService.post('rest/analysis/vehicle/collision',filter);
