define(function() {
	return ['$scope', 'i18nService', '$lt', '$filter', 'GridService', 'GridServicesCopy', 'ModalService', '$element', 'localSession', 'HttpService', 'DialogService', 'SystemService', 'DateFormat','EmptyInput','delEmptyInput', AccompanyImsiCtrl];

	function AccompanyImsiCtrl($scope, i18nService, $lt, $filter, GridService, GridServicesCopy, ModalService, $element, localSession, HttpService, DialogService, SystemService, DateFormat,EmptyInput,delEmptyInput) {
		$scope.btn = {
	        status : true
	    }
		$scope.showDel = false;
		...
		var exports = function(){
			...
			var filter = {
				title: "IMSI-MAC关联分析-"+times,
			    fields: {},
			    fileName: "AccompanyImsiPageList-"+times+".xlsx",
			    sheetName: "Sheet1",
			    param: {
			...
			if($scope.addressType == 'imsi' || !$scope.addressType){
				...
				delete filter.param.mac;
				filter.fields = field_imsi;
				HttpService.post('rest/analysis/wifi/accompany_imsi_export',filter).then(function success(resp) {
					$scope.showLoading = false;
				...
				filter.param.mac = $scope.num;
				delete filter.param.imsi;
				filter.fields = field_mac;
				HttpService.post('rest/analysis/wifi/accompany_mac_export',filter).then(function success(resp) {
					$scope.showLoading = false;
	            	var filter={fileName: resp.file}
								...
								var list = resp.items;						
				           		...
				           		$scope.searchDetails = function(row){
									ModalService.showModal({
							            ...
							            controller: 'AccompanyImsiDetailCtrl',
							            inputs: { 
							            	row : row,
							                data : postData
							            }
