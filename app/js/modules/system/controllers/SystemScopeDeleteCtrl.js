define(['../../common/utils/md5'],function(md5) {
		return ['$scope', 'HttpService', 'SystemService', 'title', 'DialogService', 'parentID', 'close', SystemScopeDeleteCtrl];
		function SystemScopeDeleteCtrl($scope, HttpService, SystemService, title, DialogService, parentID, close) {
			$scope.close = close;
			$scope.title = title;
			$scope.del = {};
			$scope.atype = 'only';
				...
				DialogService.showConfirm(
	                ...
	                '确认删除后将无法恢复！',function(){
	                	...
	                	checkInfo.then(function success(resp){
	                		if($scope.atype == 'only'){
	                			HttpService.get('rest/system/domain/delete',{adCode:parentID}).then(function success(data){
	                				var del = {};
		    						del.adCode = '00';
	                			...
	                			HttpService.get('rest/system/domain/clean',{adCode:parentID}).then(function success(data){
	                				var clean = {};
		    						clean.adCode = '00';
