define(
	function () {
		return ['$scope', 'HttpService', 'SystemService','close','title', 'datas', BlacklistEditCtrl];
		function BlacklistEditCtrl($scope, HttpService, SystemService,close,title,datas) {
			
			$scope.title = title;
			$scope.addBlacklist = datas;
			$scope.imsiList = datas.imsiList;
			$scope.imeiList = datas.imeiList;
			$scope.macList = datas.macList;
			$scope.plateList = datas.plateList;
