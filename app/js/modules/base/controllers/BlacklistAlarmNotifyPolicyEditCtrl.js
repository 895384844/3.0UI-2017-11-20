define(
	function () {
		return ['$scope', 'HttpService','$http', 'SystemService','GridService','GridServices','title','forward','close', BlacklistAlarmNotifyPolicyEditCtrl];
		function BlacklistAlarmNotifyPolicyEditCtrl($scope, HttpService,$http,SystemService,GridService,GridServices,title, forward,close) {
			...
			$scope.data = {
				id : $scope.forward.id,
				terminalId : $scope.forward.terminal.id,
				peopleId : $scope.forward.people.id
			};			
			
			$scope.terminalName = forward.terminal.name;
			$scope.peopleName = forward.people.name;
			
			$scope.sure = function(){
				SystemService.saveOrUpdateUser('rest/surveillance/targetpolicy/edit',$scope.data).then(function success(data){
					close();
				})
			};
			...
			$scope.terminaShow = function(){
				...
				$scope.gridOptions.additionalAction = function(gridApi) {
					gridApi.selection.on.rowSelectionChanged($scope,function(){
			            ...
			            $scope.data.terminalId = selection[0].id;
			        });
				}
			};
			...
			$scope.peopleShow = function(){
				...
				if($scope.selTerminal.show){				
					...
					$scope.getPagingList=function(currentPage, pageSize,sort){
			   		...
			   		GridServices.create($scope,{
						fetchData:true,
						columnDefs:[
					        { field: 'name',displayName: '人员名称',enableSorting:false,enableColumn:false,enableColumnMenu:false},
				...
				$scope.gridOption.additionalAction = function(gridApi) {
					gridApi.selection.on.rowSelectionChanged($scope,function(){
			            var selection = $scope.gridApi.selection.getSelectedRows();
			            $scope.peopleName = selection[0].name;
